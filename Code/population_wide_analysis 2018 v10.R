#########                   MAIN ANALYSIS FOR POPULATION WIDE STUDY            ###############

#### 
#   exposure variables: PM2.5 - using SEDAC data (2010-2016), and CO, NO2, Ozone - using EPA monitoring station data(2010-2018))
#   outcome: ED visits per day (2010-2016 for PM2.5, through 2018 for others)
...
####            note, this db comes from_raw_to_final_dataframe.R, and excludes those who need to be excluded
...
load("C:/Users/pegeorg/OneDrive - Emory University/Papers and Projects/Fellowship Project/DB/Saved R Dataframes/ED_DF_Finalized/SCD_ER_count_10m_df_2018.rda")
...
####            first, load the main outcome variable, which is total number of ED visits per day
####            note, this db comes from raw to final dataframe, and excludes those who need to be excluded
load("C:/Users/pegeorg/OneDrive - Emory University/Papers and Projects/Fellowship Project/DB/Saved R Dataframes/ED_DF_Finalized/SCD_ER_count_5m_df_2018.rda")
...
ED_model_results.df$distance <- factor(ED_model_results.df$distance, 
                                          levels = c('5 mile radius', '10 mile radius', '20 mile radius'))
...
write.csv(ED_model_results.df, file = '../Analyses/Main_Model_2018.csv')
...
ggplot(ED_model_results.df, aes(Pollutant , IRR , color = distance)) +
  ...
  scale_color_manual(name="Distance from CHOA",values=c("coral","steelblue", '#009E73')) +
  scale_shape_manual(name="Distance from CHOA",values=c(17,19, 18)) +
  theme_classic() +
  ...
  labs(title = 'Figure 2. Effect of pollutants on ED Visits, by capture area', 
       subtitle = 'Main exposure is individual pollutant (3 day rolling mean), main outcome is daily number of emergency department visits', 
       caption = 'Plot above shows results of 12 separate models (4 pollutants x 3 areas). Pollutant values standardized (mean centered and divided by 2 times their 
standard deviation). Thus, PM2.5 20 mile radius incidence rate ratio of 1.05 can be interpreted as for every 2 standard deviation change in 3 day 
rolling mean PM2.5 levels, the daily number of emergency visits in this cohort increases by 5%.') +
